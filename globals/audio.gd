...
func _ready() -> void:
	EventBus.stick_hit_ball.connect(func(a: Ball, power: float): create_sound(SoundType.StickHit, a.global_position, 0.5 + 0.5 * power / 100, randf_range(0.9,1.1)))
	EventBus.ball_hit_wall.connect(func(a: Ball): create_sound(SoundType.WallHit, a.global_position, sqrt(a.velocity.length()) / 10, randf_range(0.9,1.1)))
	EventBus.ball_hit_ball.connect(func(a: Ball, b: Ball): create_sound(SoundType.BallHit, a.global_position, max(a.velocity.length(), b.velocity.length()) / 100, randf_range(0.9,1.1)))
